...
const Projects = () => {
    ...
    return (
        <section id="projects" className="py-12 md:py-20 px-4 md:px-20 bg-black">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
                    {projects.map((project, index) => (
                        ...
                        >
                            ...
                            <div className="relative border border-white/10 group-hover:border-transparent rounded-xl md:rounded-2xl overflow-hidden transition-all duration-500 h-full flex flex-col">
                                {/* Image Container */}
                                ...
                                <div className="p-4 md:p-6 flex-1 flex flex-col">
                                    ...
                                    <p className="mb-3 md:mb-4 text-xs md:text-sm text-gray-400 line-clamp-3">
                                        {project.description}
                                    </p>

                                    {/* Features List - Hidden on mobile to save space */}
                                    <ul className="hidden md:block mb-4 text-xs space-y-1 flex-1 text-gray-500">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 md:gap-2 mb-3 md:mb-4">
                                        {project.tags.slice(0, 3).map((tag, tagIndex) => (
                                            <span
                                                key={tagIndex}
                                                className="px-2 py-0.5 md:py-1 text-[10px] md:text-xs font-medium rounded-full transition-all duration-300 bg-white/10 text-white/80 group-hover:bg-white/20"
                                            >
                                                {tag}
                                            </span>
                                        ))}
                                        {project.tags.length > 3 && (
                                            <span className="px-2 py-0.5 md:py-1 text-[10px] md:text-xs font-medium rounded-full text-gray-500">
                                                +{project.tags.length - 3}
                                            </span>
                                        )}
                                    </div>
